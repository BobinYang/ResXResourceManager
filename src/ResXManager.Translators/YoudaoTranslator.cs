...
[DataTemplate(typeof(YoudaoTranslator))]
public class YoudaoTranslatorConfiguration : Decorator
{
}
...
public class YoudaoTranslator : TranslatorBase
{
    ...
    public YoudaoTranslator()
        : base("Youdao", "Youdao", _uri, _credentialItems)
    {
    }
    ...
    [DefaultValue("https://openapi.youdao.com/api")]
    ...
    protected override async Task Translate(ITranslationSession translationSession)
    {
        ...
        {
            translationSession.AddMessage("Youdao Translator requires APP Key.");
            return;
        }
        ...
        foreach (var languageGroup in translationSession.Items.GroupBy(item => item.TargetCulture))
        {
            ...
            while (true)
            {
                ...
                parameters.AddRange(new[]
                {
                    ...
                    "from", YoudaoLangCode(translationSession.SourceLanguage),
                    "to", YoudaoLangCode(targetCulture),
                    "signType", "v3",
                    "appKey", appKey,
                    "salt", salt,
                    "sign", sign,
                ...
                // Call the Youdao API
                var response = await GetHttpResponse<YoudaoTranslationResponse>(
                    apiUrl,
                    parameters,
                ...
                {
                    translationSession.AddMessage("Youdao Translator Error " + response.ErrorCode + ":" + GetErrorName(response.ErrorCode));
                    return;
                }
                await translationSession.MainThread.StartNew(() =>
                {
                    if (response.Translation == null) return;
                    foreach (var tuple in sourceItems.Zip(response.Translation ?? Array.Empty<string>(),
                                 (a, b) => new Tuple<ITranslationItem, string?>(a, b)))
                    {
                        tuple.Item1.Results.Add(new TranslationMatch(this, tuple.Item2, Ranking));
                    }
                }).ConfigureAwait(false);

                Thread.CurrentThread.Join(1000);//间隔一秒再请求
            }
        }
    }
    ...
    private static string YoudaoLangCode(CultureInfo cultureInfo)
    {
...
#pragma warning disable CA2016 // Forward the 'CancellationToken' parameter to methods => not available in NetFramework
    ...
    [DataContract]
    private sealed class YoudaoTranslationResponse
    {
        ...
        public string? SpeakUrl { get; set; }//查询文本的发音地址
